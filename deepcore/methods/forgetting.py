...
class Forgetting(EarlyTrain):
    def __init__(self, loader, configs, args, robust_learner, fraction=0.5, random_seed=None, epochs=200, balance=True, **kwargs):
        super().__init__(loader, configs, args, robust_learner, fraction, random_seed, epochs)
    ...
    def select(self, **kwargs):
        ...
        for c in range(self.num_classes):
            c_indx = np.arange(self.n_train)[self.loader.train_dataset.targets == c]
            budget = round(self.fraction * len(c_indx))
            top_examples = np.append(top_examples, c_indx[np.argsort(self.forgetting_events[c_indx].cpu().numpy())[::-1][:budget]])

        return {"indices": top_examples.copy(), "scores": self.forgetting_events}, configs
