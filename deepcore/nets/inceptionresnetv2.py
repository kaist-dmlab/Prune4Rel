...
from .nets_utils import Conv2d, Reduction_A
...
class Inception_ResNetv2(nn.Module):
    def __init__(self, in_channels=3, classes=50, im_size=224, pretrained=False, k=256, l=256, m=384, n=384):
        ...
        self.embedding_recorder = EmbeddingRecorder(True)
...
class Inception_ResNet_Extractor(Inception_ResNetv2):
    def __init__(self, in_channels=3, classes=50, im_size=224, pretrained=False, k=256, l=256, m=384, n=384):
    ...
    def forward(self, x):
        ...
        x = self.conv(x)
        x = self.global_average_pooling(x)
        out_final = x.view(x.size(0), -1)

        return out_final


def Inception_ResNetv2_Extractor(num_classes:int):
    return Inception_ResNet_Extractor(classes=num_classes)