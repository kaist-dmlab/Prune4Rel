...
class MyCIFAR100(Dataset):
    def __init__(self, file_path, download, noise_type='real', noise_rate = 0.2, mode='train'):
        ...
        self.cifar100_train = datasets.CIFAR100(file_path, train=True, download=download)
        self.cifar100_test = datasets.CIFAR100(file_path, train=False, download=download)
        ...
        self.clean_targets = np.array(self.cifar100_train.targets).copy()
        self.targets = np.array(self.cifar100_train.targets)
        self.n_class = len(self.cifar100_train.classes)
        ...
        if noise_type == 'clean':
            ...
            # Noise Injection
            print("CIFAR100, Noise Injection!")
            ...
            if noise_type == 'sym':  # Symmetric Noise
                ...
                for i in range(self.n_train):
                    if i in noise_idx:
                        ...
                        noise_label.append(train_label[i])
            elif noise_type =='real':  # Real Noise
                print("Inject Real Noise!")
                noise_labels = torch.load(file_path + 'CIFAR-100_human.pt')
                noise_label= noise_labels['noisy_label']
    ...
    def __getitem__(self, index):
        if self.mode == 'test':
            img, target = self.cifar100_test[index]
            img = self.test_transform(img)  # test transform
            return img, target, index
        else:
            img, _ = self.cifar100_train[index]
    ...
    def __len__(self):
        if self.mode == "test":
            ...
            return len(self.cifar100_train)
...
class cifar100_dataloader():
    def __init__(self, args, file_path, download, noise_type='real', noise_rate = 0.2):
        ...
        self.train_dataset = MyCIFAR100(self.file_path, self.download, self.noise_type, self.noise_rate, mode='train')
        self.test_dataset = MyCIFAR100(self.file_path, self.download, self.noise_type, self.noise_rate, mode='test')
    ...
    def run(self, mode, pred=[], prob=[]):
        if mode == 'train':
            ...
            test_dataset = MyCIFAR100(self.file_path, self.download, self.noise_type, self.noise_rate, mode)
